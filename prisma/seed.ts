...
async function main() {
  ...
  // SEED_DEMO=false (production) skips the sample user, posts, likes, comments and forum topic.
  const withDemo = process.env.SEED_DEMO !== "false";
  if (!adminPlain || adminPlain.length < 12 || (withDemo && (!userPlain || userPlain.length < 12))) {
    throw new Error(
      "Defina SEED_ADMIN_PASSWORD e SEED_USER_PASSWORD (mínimo 12 caracteres) no .env antes de rodar o seed. " +
        "Com SEED_DEMO=false, só SEED_ADMIN_PASSWORD é necessária."
    );
  }

  const adminPassword = await bcrypt.hash(adminPlain, 12);

  // Use a real inbox in production so the admin can receive password-reset e-mails.
  const adminEmail = (process.env.SEED_ADMIN_EMAIL || "admin@conectax.local").trim().toLowerCase();
  ...
  if (withDemo) {
    const member = await db.user.upsert({
      ...
      create: {
        ...
        passwordHash: await bcrypt.hash(userPlain!, 12),
        role: "USER",
  ...
  console.log("----------------------------------------");
  console.log(`Login admin: ${adminEmail} (senha: SEED_ADMIN_PASSWORD)`);
  if (withDemo) console.log("Login usuário: usuario@conectax.local (senha: SEED_USER_PASSWORD)");
  console.log("----------------------------------------");
}
